# Building a RAG System with Rig 0.24.0 in Under 100 Lines of Code
...
- `rig-core`: The main Rig library for building LLM applications (version 0.24.0)
...
Next, we'll create an in-memory vector store to hold our documents. In Rig 0.24.0, the API has been simplified:

```rust
use rig::vector_store::in_memory_store::InMemoryVectorStore;
```

The `InMemoryVectorStore` is a simple vector store provided by Rig that keeps all data in memory. This is suitable for small to medium-sized document collections. For larger collections, you might want to use a persistent storage solution like MongoDB, LanceDB, Neo4j, or Qdrant (available as companion crates).
...
**Key change in Rig 0.24.0:** Instead of creating an empty vector store and then adding documents, we now use `InMemoryVectorStore::from_documents()` to create the vector store directly from our embeddings. This is more concise and efficient.

### Step 4: Building the RAG agent

With our document store populated, we can now create our RAG agent. In Rig 0.24.0, the agent builder API has changed:
...
The `preamble` sets the behavior of our agent, and `dynamic_context(2, ...)` tells the agent to use the top 2 most relevant documents from our vector store for each query.

### Step 5: Using Rig's Built-in CLI Interface

One of the great features of Rig is its built-in utilities that simplify common tasks. We can use Rig's `cli_chatbot` function to create an interactive interface:
...
This simple function call provides a robust CLI interface with built-in chat history functionality.

## Putting It All Together

Now that we've gone through each component, let's look at the complete code for our RAG system using Rig 0.24.0:
...
> how do the ideas of both texts relate to each other?
========================== Response ============================
...
## What's New in Rig 0.24.0?

Rig 0.24.0 brings several improvements to the API that make building RAG systems even more straightforward:

1. **Simplified Agent Builder**: The `.agent()` method provides a more consistent interface across all agent types, whether they use RAG, tools, or both.

2. **Improved Vector Store Initialization**: The `InMemoryVectorStore::from_documents()` method allows you to create and populate a vector store in one step.

3. **Better Type Safety**: Enhanced type system ensures compile-time correctness in LLM interactions.

4. **Expanded Provider Support**: Rig 0.24.0 supports 20+ model providers under one unified interface, including OpenAI, Anthropic, Cohere, Google Gemini, xAI, and DeepSeek.

5. **GenAI Semantic Convention Compatibility**: Full support for observability and tracing in LLM applications.
...
1. **Scalability**: For larger document collections, consider using a dedicated vector store instead of the in-memory store. Rig 0.24.0 supports MongoDB, LanceDB, Neo4j, and Qdrant through companion crates. Visit the [Rig Repository](https://github.com/0xPlaygrounds/rig) to explore these options.
...
5. **Monitoring and Logging**: Implement comprehensive logging and monitoring to track system performance and user interactions. Rig 0.24.0 includes built-in support for OpenTelemetry and platforms like Langfuse.
...
8. **Model Selection**: Consider upgrading to GPT-4 or other more capable models for production use, as they provide better reasoning and more accurate responses.

By addressing these considerations, you can turn this proof-of-concept into a robust, production-ready system.

## Next Steps

Now that you've built a basic RAG system with Rig 0.24.0, here are some ways to extend and improve it:

- **Add Document Chunking**: Split large documents into smaller chunks for more precise retrieval
- **Implement Multi-Modal RAG**: Add support for images and other media types
- **Add Tools**: Give your agent access to external tools like calculators, web search, or APIs
- **Experiment with Different Models**: Try different embedding models and LLMs to find the best performance for your use case
- **Build a Web Interface**: Replace the CLI with a web-based UI using a framework like Actix-web or Rocket

The Rig ecosystem is continuously growing, and the community is active on [Discord](https://discord.gg/playgrounds) and [GitHub](https://github.com/0xPlaygrounds/rig). Join us to share your projects, get help, and contribute to the future of LLM development in Rust!
